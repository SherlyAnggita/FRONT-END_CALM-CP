...
function NotifSkeleton() {
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-white/30 bg-white/30 p-4 backdrop-blur-sm dark:border-white/10 dark:bg-white/5">
...
function NotifCard({ notif, onRead, style }) {
  ...
  return (
    <div
      ...
      style={style}
      className={`group relative flex cursor-pointer flex-col gap-2.5 rounded-2xl border p-4 backdrop-blur-sm transition-all duration-200
        hover:-translate-y-[1px] hover:shadow-md active:scale-[0.985]
        ${
          isUnread
            ? "border-primary/30 bg-white/50 dark:border-primary/25 dark:bg-white/10"
            : "border-white/30 bg-white/30 hover:bg-white/50 dark:border-white/10 dark:bg-white/5 dark:hover:bg-white/10"
        }`}
    >
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          {isUnread ? (
            ...
            <span className="h-2 w-2 flex-shrink-0 rounded-full bg-base-300 dark:bg-white/20" />
          )}
          <span
      ...
      <p className="line-clamp-3 flex-1 text-xs leading-relaxed text-slate-500 dark:text-slate-400">
        {notif.message}
      </p>
...
function EmptyState({ filter }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-white/30 bg-white/30 py-20 backdrop-blur-sm dark:border-white/10 dark:bg-white/5">
      <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/50 text-slate-400 dark:bg-white/10 dark:text-slate-500">
...
export default function NotificationsPage() {
  ...
  return (
    <div className="min-h-screen">
      {/* Sticky Header */}
      <div className="sticky top-0 z-20 border-b border-white/20 bg-white/30 backdrop-blur-lg dark:border-white/10 dark:bg-white/5">
        <div className="mx-auto max-w-5xl px-4 pb-3 pt-4 md:px-8">
          <div className="flex items-center justify-between gap-3">
            ...
            {unreadCount > 0 && (
              <button
                ...
                className="flex items-center gap-1.5 rounded-xl border border-primary/20 bg-white/50 px-3 py-1.5 text-xs font-semibold text-primary transition-all duration-150 hover:bg-white/70 active:scale-95 disabled:opacity-60 dark:border-primary/25 dark:bg-white/10 dark:hover:bg-white/20"
              >
                {loadingReadAll ? (
          ...
          {/* Filter tabs */}
          <div className="mt-3 flex gap-1 rounded-xl bg-white/40 p-1 shadow-sm backdrop-blur-sm dark:bg-white/10">
            {FILTERS.map((f) => (
              <button
                key={f.value}
