...
import { updateMoodLabel } from "../../../services/Admin/moodLabel";

export default function UpdateMoodLabel({
  isOpen,
  onClose,
  isDark,
  colors,
  ...
  useEffect(() => {
    if (isOpen && initialData) {
      setForm({
        ...
        paperColor: initialData.paperColor || "#FDE68A",
      });
    }
  }, [isOpen, initialData]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await updateMoodLabel(initialData.id, payload);

      toast.success(res?.message || "Mood label berhasil diperbarui");
      onClose();
      await refetch();
    } catch (err) {
      toast.error(err?.message || "Gagal memperbarui mood label");
    } finally {
