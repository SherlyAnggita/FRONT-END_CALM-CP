      ...
      {entries.map((entry) => (
        ...
        >
          <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div className="inline-flex w-fit items-center gap-2 rounded-full bg-white/70 px-4 py-1.5 text-m font-medium text-[#6d5ba3] backdrop-blur-sm dark:bg-slate-900/70 dark:text-slate-100">
              <span>{entry.moodLabel?.emoji || "🙂"}</span>
              <span>{entry.moodLabel?.name || "Mood"}</span>
            </div>

            <p className="w-fit rounded-full bg-white/60 px-3 py-1 text-xs font-medium text-[#7c7285] dark:bg-slate-900/70 dark:text-slate-300">
              {formatMoodDate(entry.entryDate)}
            </p>
          </div>

          <div className="rounded-2xl border border-[#0a4174] bg-[#ffffff] p-4 backdrop-blur-sm dark:border-slate-600 dark:bg-slate-900">
            <p className="text-sm leading-relaxed text-[#51465a] dark:text-slate-200">
              {entry.feelingText}
            </p>
          </div>
